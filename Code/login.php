...
$query=mysqli_query($db,"SELECT * FROM `patient_details` WHERE `email`= '$email' && `password`= '$psw' ") or die(mysqli_error($db));
...
if($name!=NULL)
{
$query=mysqli_query($db,"UPDATE `patient_login` SET `id`='$id' WHERE 1") or die(mysqli_error($db));

    
    
 echo ' <script language="javascript" type="text/javascript">
alert("Hello Mr/Mrs. '.$name.' You are logged in");
